    ...
    def StartOfScan(self, data, hdrlen):
        ...
        for y in range(self.height // 8):
            for x in range(self.width // 8):
                # decode 8x8 block
                matL, oldlumdccoeff = self.BuildMatrix(st, 0, self.quant[
                    self.quantMapping[0]], oldlumdccoeff)
                matCr, oldCrdccoeff = self.BuildMatrix(st, 1, self.quant[
                    self.quantMapping[1]], oldCrdccoeff)
                matCb, oldCbdccoeff = self.BuildMatrix(st, 1, self.quant[
                    self.quantMapping[2]], oldCbdccoeff)
                # store it as RGB
                for yy in range(8):
                    for xx in range(8):
...
text = ["class HuffmanTable {",
    "\tfield int Type;",
    "\tfield int CurrentCode;",
    "\tconstructor HuffmanTable new(int type){",
        "\t\tlet Type = type;",
        "\t\tlet CurrentCode = -1;",
        "\t\treturn this;",
    "\t}",
    "\tmethod int GetCode(Stream st){",
        "\t\tlet CurrentCode = CurrentCode + 1;",
        "\t\tif (Type = 0) {",
            "\t\t\treturn GetCode0();",
        "\t\t}",
        "\t\tif (Type = 16) {",
...
huffman0Dict = dict()
for index, code in enumerate(huffman0):
    if code not in huffman0Dict.keys():
        huffman0Dict[code] = []
    huffman0Dict[code].append(index)

huffman16Dict = dict()
for index, code in enumerate(huffman16):
    if code not in huffman16Dict.keys():
        huffman16Dict[code] = []
    huffman16Dict[code].append(index)

for code in huffman0Dict.keys():
    text.append(f"\t\tif ({' | '.join(f'(CurrentCode = {index})' for index in huffman0Dict[code] if index < 32727)}) {{ return {code}; }}")

text += ["\t\treturn -1;",
    "\t}",
    "\tmethod int GetCode16() {"]

for code in huffman16Dict.keys():
    text.append(f"\t\tif ({' | '.join(f'(CurrentCode = {index})' for index in huffman16Dict[code] if index < 32727)}) {{ return {code}; }}")
